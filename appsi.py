...
with tabs[17]:
    ...
    modo = st.radio(
        "tipo de exercício",
        [
            "taxa nominal → efetiva / equivalente",
            "taxa real",
    ...
    if modo == "taxa nominal → efetiva / equivalente":
        st.markdown("**1) transformar nominal em efetiva da capitalização**")
        ...
        st.markdown("**2) equivalente para a unidade que você quer**")
        un_dest = st.selectbox("unidade desejada", list(PER_YEAR.keys()), index=6, key="sim_nom4")
        taxa_equiv = equivalent_rate(taxa_eff_cap, cap, un_dest)
        st.metric("taxa equivalente final", f"{pct(taxa_equiv)} {un_dest}")

        if show_formula:
            formula_box(
                f"taxa efetiva da capitalização = {taxa_nom:.6f} × ({PER_YEAR[un_nom]}/{PER_YEAR[cap]}) = {taxa_eff_cap:.6f}% {cap}\n"
                f"taxa equivalente final = (1+{taxa_eff_cap/100:.8f})^({PER_YEAR[un_dest]}/{PER_YEAR[cap]}) - 1 = {taxa_equiv:.6f}% {un_dest}"
            )
        if show_hp:
            hp_box("pela hp12c", [
                f"1 + {taxa_eff_cap/100:.10g}",
                f"ENTER {PER_YEAR[un_dest]/PER_YEAR[cap]:.10g}",
                "y^x",
                "1 -"
            ])
...
with tabs[4]:
    ...
    eff_annual = annual_effective_from_rate(eff_cap, cap)*100
    st.metric("taxa efetiva na unidade da capitalização", f"{pct(eff_cap)} {cap}")
    st.metric("taxa efetiva anual", pct(eff_annual))
    if show_formula:
        formula_box(
            f"1) taxa efetiva da capitalização = {nominal:.6f} × ({PER_YEAR[cap]}/{PER_YEAR[un_taxa]}) = {eff_cap:.6f}% {cap}\n"
            f"2) taxa efetiva anual = (1+{eff_cap/100:.8f})^{PER_YEAR[cap]} - 1 = {eff_annual:.6f}% a.a."
        )
    if show_hp:
        hp_box("pela hp12c", [
            f"{eff_cap:.10g} i",
