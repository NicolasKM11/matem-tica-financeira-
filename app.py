...
with tabs[2]:
    ...
    with c2:
        st.write(f"2% a.b → semestre = {pct(simple_proportional(2, 'ao bimestre', 'ao semestre'),2)}")
        st.write(f"3% a.m → ano = {pct(simple_proportional(3, 'ao mês', 'ao ano'),2)}")
        st.write(f"18% a.a → mês = {pct(simple_proportional(18, 'ao ano', 'ao mês'),4)}")
