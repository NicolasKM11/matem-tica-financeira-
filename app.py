...
def equivalent_rate(rate_pct, from_unit, to_unit):
    i = rate_pct / 100
    # exemplo: 18% a.a. equivalente ao trimestre -> (1+0,18)^(1/4)-1
    return (((1 + i) ** (PER_YEAR[from_unit] / PER_YEAR[to_unit])) - 1) * 100
...
with tabs[3]:
    ...
    if show_formula:
        formula_box(
            f"i_destino = (1+{taxa/100:.8f})^({PER_YEAR[de]}/{PER_YEAR[para]}) - 1\n"
            f"i_destino = {eq:.6f}%"
        )
    if show_hp:
        hp_box("pela hp12c", [
...
with tabs[7]:
    ...
    un_taxa = st.selectbox("unidade da taxa i", list(PER_YEAR.keys()), index=3, key="drs_ut")
    prazo = st.number_input("tempo", min_value=0.0, value=8.0, key="drs_t")
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="drs_tt")
    n = convert_time(prazo, un_tempo, un_taxa)
    st.caption(f"n convertido para a unidade da taxa = {n:.6f} {un_taxa}(s)")
    if modo == "PV e D":
        i = st.number_input("taxa i (%)", value=3.0, key="drs_i1")
        ...
        PV = st.number_input("PV", min_value=0.0, value=9600.0, key="drs_pv2")
        i = ((FV/PV)-1)/n*100 if n else 0.0
        st.metric("taxa i", pct(i) + f" {un_taxa}")
    else:
        PV = st.number_input("PV", min_value=0.0, value=52419.35, key="drs_pv3")
        i = st.number_input("taxa i (%)", value=3.0, key="drs_i3")
        n_taxa = ((FV/PV)-1)/(i/100) if i else 0.0
        prazo_saida = convert_time(n_taxa, un_taxa, un_tempo)
        st.metric("prazo", f"{prazo_saida:.6f} {un_tempo}(s)")
# 9
with tabs[8]:
    ...
    if modo == "PV e D":
        ...
        un_taxa = st.selectbox("unidade da taxa d", list(PER_YEAR.keys()), index=3, key="dcs_ut1")
        prazo = st.number_input("tempo", min_value=0.0, value=8.0, key="dcs_t1")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="dcs_tt1")
        n = convert_time(prazo, un_tempo, un_taxa)
        PV, D = desconto_comercial_simples(FV, d, n)
        st.metric("D", money(D))
        st.metric("PV", money(PV))
        st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
        if show_formula:
        ...
        un_taxa = st.selectbox("unidade da taxa d", list(PER_YEAR.keys()), index=3, key="dcs_ut2")
        prazo = st.number_input("tempo", min_value=0.0, value=5.0, key="dcs_t2")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="dcs_tt2")
        n = convert_time(prazo, un_tempo, un_taxa)
        D = FV - PV
        d = D/(FV*n)*100 if n and FV else 0.0
        st.metric("D", money(D))
        st.metric("taxa d", pct(d) + f" {un_taxa}")
    else:
        qtd = st.number_input("quantidade de títulos", min_value=2, max_value=10, value=2)
        d = st.number_input("taxa d (%)", value=15.0, key="dcs_d3")
        un_taxa = st.selectbox("unidade da taxa d", list(PER_YEAR.keys()), index=6, key="dcs_ut3")
        un_tempo = st.selectbox("unidade do tempo informado", list(TIME_FACTORS.keys()), index=1, key="dcs_tt3")
        ...
        for k in range(int(qtd)):
            ...
            prazo = st.number_input(f"tempo {k+1}", min_value=0.0, value=43.0 if k == 0 else 58.0, key=f"n_t{k}")
            n = convert_time(prazo, un_tempo, un_taxa)
            PV, D = desconto_comercial_simples(fv, d, n)
            total_pv += PV
            total_d += D
...
with tabs[9]:
    ...
    un_taxa = st.selectbox("unidade da taxa i", list(PER_YEAR.keys()), index=3, key="drc_ut")
    prazo = st.number_input("tempo", min_value=0.0, value=5.0, key="drc_t")
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="drc_tt")
    n = convert_time(prazo, un_tempo, un_taxa)
    st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
    if modo == "PV e D":
        ...
        i = (((FV/PV)**(1/n))-1)*100 if n and PV else 0.0
        st.metric("taxa i", pct(i) + f" {un_taxa}")
# 11
with tabs[10]:
    ...
    un_taxa = st.selectbox("unidade da taxa d", list(PER_YEAR.keys()), index=3, key="dcc_ut")
    prazo = st.number_input("tempo", min_value=0.0, value=8.0, key="dcc_t")
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="dcc_tt")
    n = convert_time(prazo, un_tempo, un_taxa)
    st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
    if modo == "PV e D":
        ...
        st.metric("D", money(D))
        if show_formula:
            formula_box(f"PV = FV(1-d)^n = {FV:.6f}(1-{d/100:.8f})^{n:.6f} = {PV:.6f}\nD = FV - PV = {D:.6f}")
    else:
        PV = st.number_input("PV", min_value=0.0, value=50943.32, key="dcc_pv2")
        d = (1 - (PV/FV)**(1/n))*100 if n and FV else 0.0
        st.metric("taxa d", pct(d) + f" {un_taxa}")
# 12
with tabs[11]:
    ...
    un_taxa = st.selectbox("unidade da taxa de desconto", list(PER_YEAR.keys()), index=3, key="ced_ut")
    prazo = st.number_input("tempo", min_value=0.0, value=5.0, key="ced_t")
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="ced_tt")
    n = convert_time(prazo, un_tempo, un_taxa)
    ...
    D = FV*d/100*n
    pv_sem = FV - D
    i_sem = (((FV/pv_sem)**(1/n))-1)*100 if pv_sem > 0 and n else None
    pv_com = pv_sem - FV*saldo_medio_pct/100 - FV*desp_adm_pct/100
    fv_com = FV - FV*saldo_medio_pct/100
    i_com = (((fv_com/pv_com)**(1/n))-1)*100 if pv_com > 0 and fv_com > 0 and n else None
    st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
    st.metric("valor líquido sem retenções", money(pv_sem))
    st.metric("taxa implícita sem retenções", (pct(i_sem) + f" {un_taxa}") if i_sem is not None else "indefinida")
    st.metric("valor líquido com retenções", money(pv_com))
    st.metric("taxa implícita com retenções", (pct(i_com) + f" {un_taxa}") if i_com is not None else "indefinida")
    if show_formula:
        formula_box(
            f"D = FV·d·n = {FV:.6f}×{d/100:.8f}×{n:.6f} = {D:.6f}\n"
...
with tabs[12]:
    ...
    if sub in ["postecipada", "antecipada"]:
        ...
        i = st.number_input("taxa (%)", value=1.8, key=f"su_i_{sub}")
        un_taxa = st.selectbox("unidade da taxa", list(PER_YEAR.keys()), index=3, key=f"su_ut_{sub}")
        tempo = st.number_input("tempo total", min_value=1.0, value=36.0, key=f"su_t_{sub}")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key=f"su_tt_{sub}")
        n = convert_time(tempo, un_tempo, un_taxa)
        st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
        if alvo == "PMT":
        ...
        i = st.number_input("taxa (%)", value=3.0, key="dif_i")
        un_taxa = st.selectbox("unidade da taxa", list(PER_YEAR.keys()), index=3, key="dif_ut")
        car_t = st.number_input("carência", min_value=0.0, value=2.0)
        n_t = st.number_input("tempo das parcelas", min_value=1.0, value=6.0)
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="dif_tt")
        car = convert_time(car_t, un_tempo, un_taxa)
        n = convert_time(n_t, un_tempo, un_taxa)
        pmt = deferred_annuity_pmt(pv, i, n, car, due="END")
        st.metric("PMT da série diferida", money(abs(pmt)))
        st.caption(f"carência convertida = {car:.6f} {un_taxa}(s) | n convertido = {n:.6f} {un_taxa}(s)")
        if show_formula:
            formula_box(
        ...
        un_taxa = st.selectbox("unidade da taxa e da perpetuidade", list(PER_YEAR.keys()), index=6, key="perp_ut")
        if modo == "achar PV":
        ...
        st.caption(f"resultado por {un_taxa}")
# 14
with tabs[13]:
    ...
    un_taxa = st.selectbox("unidade da taxa implícita", list(PER_YEAR.keys()), index=3, key="epi_ut")
    tempo = st.number_input("tempo total das parcelas", min_value=1.0, value=4.0)
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="epi_tt")
    n = convert_time(tempo, un_tempo, un_taxa)
    ...
    st.metric("taxa implícita", pct(taxa_impl) + f" {un_taxa}")
    st.caption(f"n convertido = {n:.6f} {un_taxa}(s)")
# 15
with tabs[14]:
    st.subheader("renegociação")
    pmt_antiga = st.number_input("prestação antiga", min_value=0.0, value=4727.98)
    restam_t = st.number_input("tempo restante da dívida antiga", min_value=1.0, value=6.0)
    i = st.number_input("taxa (%)", value=2.0)
    un_taxa = st.selectbox("unidade da taxa", list(PER_YEAR.keys()), index=3, key="ren_ut")
    un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="ren_tt")
    novas_t = st.number_input("tempo total das novas parcelas", min_value=1.0, value=18.0)
    restam = convert_time(restam_t, un_tempo, un_taxa)
    novas = convert_time(novas_t, un_tempo, un_taxa)
    ...
    st.caption(f"restante convertido = {restam:.6f} {un_taxa}(s) | novo prazo convertido = {novas:.6f} {un_taxa}(s)")
    if show_hp:
        hp_box("pela hp12c", [
            "[g] END",
...
with tabs[15]:
    ...
    un_fluxo = st.selectbox("unidade da taxa / períodos do fluxo", list(PER_YEAR.keys()), index=6, key="fc_ut")
    ...
    if len(df) > 0:
        ...
        if len(df2) == 0:
            ...
            df2 = df2.groupby("período", as_index=False)["fluxo"].sum()
            ...
            st.metric("TIR / IRR", (pct(r) + f" {un_fluxo}") if r is not None else "não encontrada")
...
with tabs[17]:
    ...
    if modo == "taxa proporcional":
        ...
        if show_formula:
            formula_box(
                f"taxa equivalente = (1+{taxa/100:.8f})^({PER_YEAR[de]}/{PER_YEAR[para]}) - 1 = {taxa_equiv:.6f}% {para}"
            )
        if show_hp:
            hp_box("pela hp12c", [
                f"1 + {taxa/100:.10g}",
                f"ENTER {PER_YEAR[de]/PER_YEAR[para]:.10g}",
                "y^x",
                "1 -"
            ])
        ...
        st.metric("taxa efetiva da capitalização", f"{pct(taxa_eff_cap)} {cap}")
        if show_formula:
            formula_box(
                f"taxa efetiva = {taxa_nom:.6f} × ({PER_YEAR[un_nom]}/{PER_YEAR[cap]}) = {taxa_eff_cap:.6f}% {cap}"
            )
        ...
        alvo = st.radio("descobrir", ["valor recebido e desconto", "taxa de desconto"], horizontal=not watch_mode, key="sim_dcs_sub")
        un_taxa = st.selectbox("unidade da taxa d", list(PER_YEAR.keys()), index=3, key="sim_dcs_ut")
        prazo = st.number_input("tempo", min_value=0.0, value=8.0 if alvo=="valor recebido e desconto" else 5.0, key="sim_dcs_t")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="sim_dcs_tt")
        n = convert_time(prazo, un_tempo, un_taxa)
        if alvo == "valor recebido e desconto":
            FV = st.number_input("valor nominal / FV", min_value=0.0, value=65000.0, key="sim_dcs_1")
            d = st.number_input("taxa d (%)", min_value=0.0, value=3.0, key="sim_dcs_2")
            PV, D = desconto_comercial_simples(FV, d, n)
        ...
        un_taxa = st.selectbox("unidade da taxa i", list(PER_YEAR.keys()), index=6, key="sim_drc_ut")
        prazo = st.number_input("tempo", min_value=0.0, value=2.0, key="sim_drc_3")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=6, key="sim_drc_tt")
        n = convert_time(prazo, un_tempo, un_taxa)
        PV, D = desconto_racional_composto(FV, i, n)
        ...
        i = st.number_input("taxa (%)", min_value=0.0, value=1.8 if subtipo=="postecipada" else 1.0, key="sim_su_1")
        un_taxa = st.selectbox("unidade da taxa", list(PER_YEAR.keys()), index=3, key="sim_su_ut")
        tempo = st.number_input("tempo total", min_value=1.0, value=36.0 if subtipo=="postecipada" else 360.0, key="sim_su_2")
        un_tempo = st.selectbox("unidade do tempo", list(TIME_FACTORS.keys()), index=3, key="sim_su_tt")
        n = convert_time(tempo, un_tempo, un_taxa)
        ...
        un_fluxo = st.selectbox("unidade da taxa / períodos do fluxo", list(PER_YEAR.keys()), index=6, key="sim_fc_ut")
